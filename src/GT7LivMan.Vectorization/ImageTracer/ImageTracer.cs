...
namespace ImageTracerNet
{
    public static class ImageTracer
    {
        ...
        private static TracedImage PaddedPaletteImageToTraceData(Bitmap image, Tracing tracing, SvgRendering rendering)
        {
            ...
            var colors = image.ChangeFormat(PixelFormat.Format32bppArgb).ToColorReferences().ToList();
            // Layering.Convert allocates one full (width+1)x(height+1) node grid per PALETTE color,
            // not per color actually present — with the fixed 256-color Palette below that's 256
            // full-image-sized allocations every call regardless of image content, which is what
            // made this take 15+ seconds and ~700 MB even for a simple few-color image. Using only
            // the colors actually present in this image (already reduced upstream by
            // GT7LivMan.Vectorization's own quantization) instead of the generic 256-color Palette
            // fixes both: far fewer allocations, and FindClosest below degenerates to an exact match.
            var imagePalette = colors
                .GroupBy(c => (c.A, c.R, c.G, c.B))
                .Select(g => g.First())
                .ToList();
            var colorGroups = ColorGrouping.Convert(colors, image.Width, image.Height, imagePalette);
            // 2. Layer separation and edge detection
            var rawLayers = Layering.Convert(colorGroups, image.Width, image.Height, imagePalette);
            // 3. Batch pathscan
